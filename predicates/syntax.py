...
class Term:
    ...
    def __repr__(self) -> str:
        ...
        if is_constant(self.root) or is_variable(self.root):
            return self.root
        return self.root + '(' + ','.join(str(argument)
    ...
    def _parse_prefix(string: str) -> Tuple[Term, str]:
        ...
        arguments = []
        assert is_function(name) and rest[0] == '('
        rest = rest[1:]
        while True:
            argument, rest = Term._parse_prefix(rest)
            ...
            if rest[0] == ',':
                ...
                rest = rest[1:]
                break
        return Term(name, arguments), rest
    ...
    def _parse_prefix(string: str) -> Tuple[Formula, str]:
        ...
        if is_quantifier(string[0]):
            ...
            while i < len(string) and string[i].isalnum():
                i += 1
            variable = string[1:i]
            assert is_variable(variable) and string[i] == '['
            statement, rest = Formula._parse_prefix(string[i+1:])
            assert rest[0] == ']'
            return Formula(quantifier, variable, statement), rest[1:]
